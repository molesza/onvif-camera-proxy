...
# Generated for NVRs: 192.168.6.201
# Generated on: 2025-04-17T11:04:32.124Z
...
if [ "$USE_DHCP" = true ]; then
    ...
    # Wait for IP assignment (timeout 20s)
    for i in {1..20}; do
        ip_addr=$(ip -4 addr show onv1_dbb4 | grep -oP '(?<=inet\s)\d+(\.\d+){3}')
        if [ -n "$ip_addr" ]; then
            echo "onv1_dbb4 got IP: $ip_addr"
            break
        fi
        sleep 1
    done
    if [ -z "$ip_addr" ]; then
        echo "Warning: onv1_dbb4 did not get an IP address after 20 seconds."
    fi
else
...
if [ "$USE_DHCP" = true ]; then
    ...
    # Wait for IP assignment (timeout 20s)
    for i in {1..20}; do
        ip_addr=$(ip -4 addr show onv2_628d | grep -oP '(?<=inet\s)\d+(\.\d+){3}')
        if [ -n "$ip_addr" ]; then
            echo "onv2_628d got IP: $ip_addr"
            break
        fi
        sleep 1
    done
    if [ -z "$ip_addr" ]; then
        echo "Warning: onv2_628d did not get an IP address after 20 seconds."
    fi
else
...
if [ "$USE_DHCP" = true ]; then
    ...
    # Wait for IP assignment (timeout 20s)
    for i in {1..20}; do
        ip_addr=$(ip -4 addr show onv3_fad9 | grep -oP '(?<=inet\s)\d+(\.\d+){3}')
        if [ -n "$ip_addr" ]; then
            echo "onv3_fad9 got IP: $ip_addr"
            break
        fi
        sleep 1
    done
    if [ -z "$ip_addr" ]; then
        echo "Warning: onv3_fad9 did not get an IP address after 20 seconds."
    fi
else
...
if [ "$USE_DHCP" = true ]; then
    ...
    # Wait for IP assignment (timeout 20s)
    for i in {1..20}; do
        ip_addr=$(ip -4 addr show onv4_2475 | grep -oP '(?<=inet\s)\d+(\.\d+){3}')
        if [ -n "$ip_addr" ]; then
            echo "onv4_2475 got IP: $ip_addr"
            break
        fi
        sleep 1
    done
    if [ -z "$ip_addr" ]; then
        echo "Warning: onv4_2475 did not get an IP address after 20 seconds."
    fi
else
...
if [ "$USE_DHCP" = true ]; then
    ...
    # Wait for IP assignment (timeout 20s)
    for i in {1..20}; do
        ip_addr=$(ip -4 addr show onv5_5093 | grep -oP '(?<=inet\s)\d+(\.\d+){3}')
        if [ -n "$ip_addr" ]; then
            echo "onv5_5093 got IP: $ip_addr"
            break
        fi
        sleep 1
    done
    if [ -z "$ip_addr" ]; then
        echo "Warning: onv5_5093 did not get an IP address after 20 seconds."
    fi
else
...
if [ "$USE_DHCP" = true ]; then
    ...
    # Wait for IP assignment (timeout 20s)
    for i in {1..20}; do
        ip_addr=$(ip -4 addr show onv6_cbb4 | grep -oP '(?<=inet\s)\d+(\.\d+){3}')
        if [ -n "$ip_addr" ]; then
            echo "onv6_cbb4 got IP: $ip_addr"
            break
        fi
        sleep 1
    done
    if [ -z "$ip_addr" ]; then
        echo "Warning: onv6_cbb4 did not get an IP address after 20 seconds."
    fi
else
...
if [ "$USE_DHCP" = true ]; then
    ...
    # Wait for IP assignment (timeout 20s)
    for i in {1..20}; do
        ip_addr=$(ip -4 addr show onv7_21cb | grep -oP '(?<=inet\s)\d+(\.\d+){3}')
        if [ -n "$ip_addr" ]; then
            echo "onv7_21cb got IP: $ip_addr"
            break
        fi
        sleep 1
    done
    if [ -z "$ip_addr" ]; then
        echo "Warning: onv7_21cb did not get an IP address after 20 seconds."
    fi
else
...
if [ "$USE_DHCP" = true ]; then
    ...
    # Wait for IP assignment (timeout 20s)
    for i in {1..20}; do
        ip_addr=$(ip -4 addr show onv8_69e3 | grep -oP '(?<=inet\s)\d+(\.\d+){3}')
        if [ -n "$ip_addr" ]; then
            echo "onv8_69e3 got IP: $ip_addr"
            break
        fi
        sleep 1
    done
    if [ -z "$ip_addr" ]; then
        echo "Warning: onv8_69e3 did not get an IP address after 20 seconds."
    fi
else
...
if [ "$USE_DHCP" = true ]; then
    ...
    # Wait for IP assignment (timeout 20s)
    for i in {1..20}; do
        ip_addr=$(ip -4 addr show onv9_2912 | grep -oP '(?<=inet\s)\d+(\.\d+){3}')
        if [ -n "$ip_addr" ]; then
            echo "onv9_2912 got IP: $ip_addr"
            break
        fi
        sleep 1
    done
    if [ -z "$ip_addr" ]; then
        echo "Warning: onv9_2912 did not get an IP address after 20 seconds."
    fi
else
...
if [ "$USE_DHCP" = true ]; then
    ...
    # Wait for IP assignment (timeout 20s)
    for i in {1..20}; do
        ip_addr=$(ip -4 addr show onv10_1b73 | grep -oP '(?<=inet\s)\d+(\.\d+){3}')
        if [ -n "$ip_addr" ]; then
            echo "onv10_1b73 got IP: $ip_addr"
            break
        fi
        sleep 1
    done
    if [ -z "$ip_addr" ]; then
        echo "Warning: onv10_1b73 did not get an IP address after 20 seconds."
    fi
else
...
if [ "$USE_DHCP" = true ]; then
    ...
    # Wait for IP assignment (timeout 20s)
    for i in {1..20}; do
        ip_addr=$(ip -4 addr show onv11_a6fc | grep -oP '(?<=inet\s)\d+(\.\d+){3}')
        if [ -n "$ip_addr" ]; then
            echo "onv11_a6fc got IP: $ip_addr"
            break
        fi
        sleep 1
    done
    if [ -z "$ip_addr" ]; then
        echo "Warning: onv11_a6fc did not get an IP address after 20 seconds."
    fi
else
...
if [ "$USE_DHCP" = true ]; then
    ...
    # Wait for IP assignment (timeout 20s)
    for i in {1..20}; do
        ip_addr=$(ip -4 addr show onv12_f9b6 | grep -oP '(?<=inet\s)\d+(\.\d+){3}')
        if [ -n "$ip_addr" ]; then
            echo "onv12_f9b6 got IP: $ip_addr"
            break
        fi
        sleep 1
    done
    if [ -z "$ip_addr" ]; then
        echo "Warning: onv12_f9b6 did not get an IP address after 20 seconds."
    fi
else
...
if [ "$USE_DHCP" = true ]; then
    ...
    # Wait for IP assignment (timeout 20s)
    for i in {1..20}; do
        ip_addr=$(ip -4 addr show onv13_c6b4 | grep -oP '(?<=inet\s)\d+(\.\d+){3}')
        if [ -n "$ip_addr" ]; then
            echo "onv13_c6b4 got IP: $ip_addr"
            break
        fi
        sleep 1
    done
    if [ -z "$ip_addr" ]; then
        echo "Warning: onv13_c6b4 did not get an IP address after 20 seconds."
    fi
else
...
if [ "$USE_DHCP" = true ]; then
    ...
    # Wait for IP assignment (timeout 20s)
    for i in {1..20}; do
        ip_addr=$(ip -4 addr show onv14_dfd2 | grep -oP '(?<=inet\s)\d+(\.\d+){3}')
        if [ -n "$ip_addr" ]; then
            echo "onv14_dfd2 got IP: $ip_addr"
            break
        fi
        sleep 1
    done
    if [ -z "$ip_addr" ]; then
        echo "Warning: onv14_dfd2 did not get an IP address after 20 seconds."
    fi
else
...
if [ "$USE_DHCP" = true ]; then
    ...
    # Wait for IP assignment (timeout 20s)
    for i in {1..20}; do
        ip_addr=$(ip -4 addr show onv15_8e96 | grep -oP '(?<=inet\s)\d+(\.\d+){3}')
        if [ -n "$ip_addr" ]; then
            echo "onv15_8e96 got IP: $ip_addr"
            break
        fi
        sleep 1
    done
    if [ -z "$ip_addr" ]; then
        echo "Warning: onv15_8e96 did not get an IP address after 20 seconds."
    fi
else
...
if [ "$USE_DHCP" = true ]; then
    ...
    # Wait for IP assignment (timeout 20s)
    for i in {1..20}; do
        ip_addr=$(ip -4 addr show onv16_321b | grep -oP '(?<=inet\s)\d+(\.\d+){3}')
        if [ -n "$ip_addr" ]; then
            echo "onv16_321b got IP: $ip_addr"
            break
        fi
        sleep 1
    done
    if [ -z "$ip_addr" ]; then
        echo "Warning: onv16_321b did not get an IP address after 20 seconds."
    fi
else
...
if [ "$USE_DHCP" = true ]; then
    ...
    # Wait for IP assignment (timeout 20s)
    for i in {1..20}; do
        ip_addr=$(ip -4 addr show onv17_dcb2 | grep -oP '(?<=inet\s)\d+(\.\d+){3}')
        if [ -n "$ip_addr" ]; then
            echo "onv17_dcb2 got IP: $ip_addr"
            break
        fi
        sleep 1
    done
    if [ -z "$ip_addr" ]; then
        echo "Warning: onv17_dcb2 did not get an IP address after 20 seconds."
    fi
else
...
if [ "$USE_DHCP" = true ]; then
    ...
    # Wait for IP assignment (timeout 20s)
    for i in {1..20}; do
        ip_addr=$(ip -4 addr show onv18_8ae4 | grep -oP '(?<=inet\s)\d+(\.\d+){3}')
        if [ -n "$ip_addr" ]; then
            echo "onv18_8ae4 got IP: $ip_addr"
            break
        fi
        sleep 1
    done
    if [ -z "$ip_addr" ]; then
        echo "Warning: onv18_8ae4 did not get an IP address after 20 seconds."
    fi
else
...
if [ "$USE_DHCP" = true ]; then
    ...
    # Wait for IP assignment (timeout 20s)
    for i in {1..20}; do
        ip_addr=$(ip -4 addr show onv19_9313 | grep -oP '(?<=inet\s)\d+(\.\d+){3}')
        if [ -n "$ip_addr" ]; then
            echo "onv19_9313 got IP: $ip_addr"
            break
        fi
        sleep 1
    done
    if [ -z "$ip_addr" ]; then
        echo "Warning: onv19_9313 did not get an IP address after 20 seconds."
    fi
else
...
if [ "$USE_DHCP" = true ]; then
    ...
    # Wait for IP assignment (timeout 20s)
    for i in {1..20}; do
        ip_addr=$(ip -4 addr show onv20_6c12 | grep -oP '(?<=inet\s)\d+(\.\d+){3}')
        if [ -n "$ip_addr" ]; then
            echo "onv20_6c12 got IP: $ip_addr"
            break
        fi
        sleep 1
    done
    if [ -z "$ip_addr" ]; then
        echo "Warning: onv20_6c12 did not get an IP address after 20 seconds."
    fi
else
...
if [ "$USE_DHCP" = true ]; then
    ...
    # Wait for IP assignment (timeout 20s)
    for i in {1..20}; do
        ip_addr=$(ip -4 addr show onv21_ad20 | grep -oP '(?<=inet\s)\d+(\.\d+){3}')
        if [ -n "$ip_addr" ]; then
            echo "onv21_ad20 got IP: $ip_addr"
            break
        fi
        sleep 1
    done
    if [ -z "$ip_addr" ]; then
        echo "Warning: onv21_ad20 did not get an IP address after 20 seconds."
    fi
else
...
if [ "$USE_DHCP" = true ]; then
    ...
    # Wait for IP assignment (timeout 20s)
    for i in {1..20}; do
        ip_addr=$(ip -4 addr show onv22_f7cb | grep -oP '(?<=inet\s)\d+(\.\d+){3}')
        if [ -n "$ip_addr" ]; then
            echo "onv22_f7cb got IP: $ip_addr"
            break
        fi
        sleep 1
    done
    if [ -z "$ip_addr" ]; then
        echo "Warning: onv22_f7cb did not get an IP address after 20 seconds."
    fi
else
...
if [ "$USE_DHCP" = true ]; then
    ...
    # Wait for IP assignment (timeout 20s)
    for i in {1..20}; do
        ip_addr=$(ip -4 addr show onv23_940e | grep -oP '(?<=inet\s)\d+(\.\d+){3}')
        if [ -n "$ip_addr" ]; then
            echo "onv23_940e got IP: $ip_addr"
            break
        fi
        sleep 1
    done
    if [ -z "$ip_addr" ]; then
        echo "Warning: onv23_940e did not get an IP address after 20 seconds."
    fi
else
...
if [ "$USE_DHCP" = true ]; then
    ...
    # Wait for IP assignment (timeout 20s)
    for i in {1..20}; do
        ip_addr=$(ip -4 addr show onv24_7e17 | grep -oP '(?<=inet\s)\d+(\.\d+){3}')
        if [ -n "$ip_addr" ]; then
            echo "onv24_7e17 got IP: $ip_addr"
            break
        fi
        sleep 1
    done
    if [ -z "$ip_addr" ]; then
        echo "Warning: onv24_7e17 did not get an IP address after 20 seconds."
    fi
else
...
if [ "$USE_DHCP" = true ]; then
    ...
    # Wait for IP assignment (timeout 20s)
    for i in {1..20}; do
        ip_addr=$(ip -4 addr show onv25_115b | grep -oP '(?<=inet\s)\d+(\.\d+){3}')
        if [ -n "$ip_addr" ]; then
            echo "onv25_115b got IP: $ip_addr"
            break
        fi
        sleep 1
    done
    if [ -z "$ip_addr" ]; then
        echo "Warning: onv25_115b did not get an IP address after 20 seconds."
    fi
else
...
if [ "$USE_DHCP" = true ]; then
    ...
    # Wait for IP assignment (timeout 20s)
    for i in {1..20}; do
        ip_addr=$(ip -4 addr show onv26_a614 | grep -oP '(?<=inet\s)\d+(\.\d+){3}')
        if [ -n "$ip_addr" ]; then
            echo "onv26_a614 got IP: $ip_addr"
            break
        fi
        sleep 1
    done
    if [ -z "$ip_addr" ]; then
        echo "Warning: onv26_a614 did not get an IP address after 20 seconds."
    fi
else
...
if [ "$USE_DHCP" = true ]; then
    ...
    # Wait for IP assignment (timeout 20s)
    for i in {1..20}; do
        ip_addr=$(ip -4 addr show onv27_1edb | grep -oP '(?<=inet\s)\d+(\.\d+){3}')
        if [ -n "$ip_addr" ]; then
            echo "onv27_1edb got IP: $ip_addr"
            break
        fi
        sleep 1
    done
    if [ -z "$ip_addr" ]; then
        echo "Warning: onv27_1edb did not get an IP address after 20 seconds."
    fi
else
...
if [ "$USE_DHCP" = true ]; then
    ...
    # Wait for IP assignment (timeout 20s)
    for i in {1..20}; do
        ip_addr=$(ip -4 addr show onv28_336f | grep -oP '(?<=inet\s)\d+(\.\d+){3}')
        if [ -n "$ip_addr" ]; then
            echo "onv28_336f got IP: $ip_addr"
            break
        fi
        sleep 1
    done
    if [ -z "$ip_addr" ]; then
        echo "Warning: onv28_336f did not get an IP address after 20 seconds."
    fi
else
...
if [ "$USE_DHCP" = true ]; then
    ...
    # Wait for IP assignment (timeout 20s)
    for i in {1..20}; do
        ip_addr=$(ip -4 addr show onv29_ec72 | grep -oP '(?<=inet\s)\d+(\.\d+){3}')
        if [ -n "$ip_addr" ]; then
            echo "onv29_ec72 got IP: $ip_addr"
            break
        fi
        sleep 1
    done
    if [ -z "$ip_addr" ]; then
        echo "Warning: onv29_ec72 did not get an IP address after 20 seconds."
    fi
else
...
if [ "$USE_DHCP" = true ]; then
    ...
    # Wait for IP assignment (timeout 20s)
    for i in {1..20}; do
        ip_addr=$(ip -4 addr show onv30_bc3a | grep -oP '(?<=inet\s)\d+(\.\d+){3}')
        if [ -n "$ip_addr" ]; then
            echo "onv30_bc3a got IP: $ip_addr"
            break
        fi
        sleep 1
    done
    if [ -z "$ip_addr" ]; then
        echo "Warning: onv30_bc3a did not get an IP address after 20 seconds."
    fi
else
...
if [ "$USE_DHCP" = true ]; then
    ...
    # Wait for IP assignment (timeout 20s)
    for i in {1..20}; do
        ip_addr=$(ip -4 addr show onv31_54b3 | grep -oP '(?<=inet\s)\d+(\.\d+){3}')
        if [ -n "$ip_addr" ]; then
            echo "onv31_54b3 got IP: $ip_addr"
            break
        fi
        sleep 1
    done
    if [ -z "$ip_addr" ]; then
        echo "Warning: onv31_54b3 did not get an IP address after 20 seconds."
    fi
else
...
if [ "$USE_DHCP" = true ]; then
    ...
    # Wait for IP assignment (timeout 20s)
    for i in {1..20}; do
        ip_addr=$(ip -4 addr show onv32_5b68 | grep -oP '(?<=inet\s)\d+(\.\d+){3}')
        if [ -n "$ip_addr" ]; then
            echo "onv32_5b68 got IP: $ip_addr"
            break
        fi
        sleep 1
    done
    if [ -z "$ip_addr" ]; then
        echo "Warning: onv32_5b68 did not get an IP address after 20 seconds."
    fi
else
